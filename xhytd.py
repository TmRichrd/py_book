...
url = "http://www.xhytd.com"
searchUrl = "http://www.xhytd.com/search.html?name="
...
def get_book_list():
    ...
    data = soup.select('#main > div.novelslist2 > ul > li > span.s2.wid > a')
    print(data)
    # 判断是否有搜索结果
    if (len(data) != 0):
        # 选择搜索结果
        ...
        booklist.encoding = "utf-8"
        booksoup = BeautifulSoup(booklist.text, 'lxml')
        result = booksoup.select("#list > dl > dd > a")
